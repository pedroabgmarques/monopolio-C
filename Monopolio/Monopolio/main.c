...
int contador = 0;// numero de lançamentos em que o valor dos dois dados e igual
...
void jogar(jogador jogadores[numeroMaximoJogadores], Casa tabuleiro[40])
{
	...
	case 10:printf("%s", tabuleiro[posicao].nome);
		
		if (jogadores[jogadorActual].estado == 1)
		{
			jogadores[jogadorActual].NumVezesSemJogar++;

			if (jogadores[jogadorActual].NumVezesSemJogar > 2)
			{
				printf("Deseja pagar 'x' para sair da prisão?");
	...
	case 30:printf("%s", tabuleiro[posicao].nome);
		jogadores[jogadorActual].posicao = 10;
		jogadores[jogadorActual].estado = 1;break;
